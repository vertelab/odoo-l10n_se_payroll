...
# odoo --database payroll_run_tests --config /etc/odoo/odoo.conf --init l10n_se_hr_payroll_twenty_six --test-tags january --test-enable --log-level test --stop-after-init -p 4443 --logfile /var/log/odoo/twenty_six_test_jan.log
...
@tagged('at_install', 'l10n_se_payroll_twenty_six', 'december')
class TestPayslipDecember(TransactionCase):
    ## Test 1
    def test_asse_net(self):   

        payslip = self.env.ref('l10n_se_hr_payroll_twenty_six.hr_payslip_asse_12')
        ...
        self.assertAlmostEqual(net_line.total, 16246.0, msg=f"Fel nettolön för Asse! Fick: {net_line.total}, förväntat: 16246.0.")
        _logger.info(f"SUCCESS: Asse net pay correct: {net_line.total}")


    # Frans slutar i Oktober så ingen spec i december

    ## Test 2
    def test_doris_net(self):

        payslip = self.env.ref('l10n_se_hr_payroll_twenty_six.hr_payslip_doris_12')

        net_line = payslip.line_ids.filtered(lambda l: l.code == 'nl')

        self.assertTrue(net_line, "Kunde inte hitta fältet 'nl' (Nettolön) för Doris.")
        self.assertAlmostEqual(net_line.total, 3500.0, msg=f"Fel nettolön för Doris! Fick: {net_line.total}, förväntat: 3500.0.")
    ...
    def test_camilla_net(self):
        ...
        payslip = self.env.ref('l10n_se_hr_payroll_twenty_six.hr_payslip_camilla_12')

        net_line = payslip.line_ids.filtered(lambda l: l.code == 'nl')

        self.assertTrue(net_line, "Kunde inte hitta fältet 'nl' (Nettolön) för Camilla.")
        self.assertAlmostEqual(net_line.total, 4633.0, msg=f"Fel nettolön för Camilla! Fick: {net_line.total}, förväntat: 4633.0.")
    ...
    def test_gustav_net(self):
        ...
        payslip = self.env.ref('l10n_se_hr_payroll_twenty_six.hr_payslip_gustav_12')

        net_line = payslip.line_ids.filtered(lambda l: l.code == 'nl')

        self.assertTrue(net_line, "Kunde inte hitta fältet 'nl' (Nettolön) för Gustav.")
        self.assertAlmostEqual(net_line.total, 19496.0, msg=f"Fel nettolön för Gustav! Fick: {net_line.total}, förväntat: 19496.0.")
    ...
    def test_helmer_net(self):
        ...
        payslip = self.env.ref('l10n_se_hr_payroll_twenty_six.hr_payslip_helmer_12')
